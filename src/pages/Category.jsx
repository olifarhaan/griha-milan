...
const Category = () => {
    ...
    if (params.type !== "sell" && params.type !== "rent") {
        navigate("*");
    }
    ...
    const fetchMoreListings = async () => {
        try {
            ...
            const q = query(collectionRef, where("type", "==", params.type), orderBy("timestamp", "desc"), startAfter(lastVisibleListing), limit(4));
    ...
    useEffect(() => {
        (
            async () => {
                try {
                    const collectionRef = collection(db, "listings");
                    const q = query(collectionRef, where("type", "==", params.type), orderBy("timestamp", "desc"), limit(8));
                    const querySnap = await getDocs(q);
