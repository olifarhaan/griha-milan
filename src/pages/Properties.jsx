...
const Properties = () => {
    ...
    const fetchMoreListings = async () => {
        try {
            ...
            const q = query(collectionRef, orderBy("timestamp", "desc"), startAfter(lastVisibleListing), limit(4));
    ...
    useEffect(() => {
        (
            async () => {
                try {
                    const collectionRef = collection(db, "listings");
                    const q = query(collectionRef, orderBy("timestamp", "desc"), limit(8));
                    const querySnap = await getDocs(q);
        ...
        <main className="">
            {
                listings ? <section className="max-w-6xl px-3 my-20 mx-auto">
                    <h2 className="text-3xl text-center font-bold text-heading"> Properties </h2>
                    <p className="cursor-pointer text-center text-primary hover:text-primaryHover hover:underline">Here are all of the properties</p>
